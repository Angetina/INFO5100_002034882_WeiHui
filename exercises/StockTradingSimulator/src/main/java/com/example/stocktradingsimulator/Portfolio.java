...
public class Portfolio {
    ...
    public boolean buy(String symbol, int shares, double price) {
        ...
        Optional<Holding> existing = findHolding(symbol);
        if (existing.isPresent()) {
            existing.get().buyMore(shares, price); // Merge by average price
        } else {
            Holding h = new Holding(symbol, shares, price);
            holdings.add(h);
        }

        return true;
    }
    public boolean sell(String symbol, int shares, double price) {
        Optional<Holding> existing = findHolding(symbol);

        // No shares or insufficient shares
        if (existing.isEmpty() || existing.get().getShares() < shares) {
            return false;
        }
        ...
        // Increase cash
        this.cashBalance += shares * price;

        // If sold out, exit the stock
        if (h.getShares() == 0) {
            holdings.remove(h);
        }
