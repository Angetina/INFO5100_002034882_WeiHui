...
public class Holding {
    ...
    public void buyMore(int additionalShares, double newPrice) {
        int totalShares = this.shares + additionalShares;
        double totalCost = (this.averagePurchasePrice * this.shares) + (newPrice * additionalShares);
        this.averagePurchasePrice = totalCost / totalShares;
        this.shares = totalShares;
    }
    public void sellShares(int sharesToSell) {
        if (sharesToSell > this.shares) {
            throw new IllegalArgumentException("Trying to sell more than you have");
        }
